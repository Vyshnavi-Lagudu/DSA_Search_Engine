...
# Loads the vocabulary and corresponding IDF (Inverse Document Frequency) values from vocab.txt and idf-values.txt.
# Stores the data in a dictionary vocab where each term is mapped to its IDF value.
def load_vocab():
    vocab = {}
...
# Loads the preprocessed documents (lists of words from each coding problem description) from document.txt.
def load_document():
...
# Loads the inverted index from inverted_index.txt. The inverted index maps each word to a list of document indices where the word appears.
...
# Loads the links to the actual coding problems from Qlink.txt.
def load_link_of_qs():
...
# This function calculates the TF-IDF score for all documents relevant to the query terms.
# It sums the TF-IDF values of each term for each document.
# The documents are then sorted in decreasing order of relevance.
